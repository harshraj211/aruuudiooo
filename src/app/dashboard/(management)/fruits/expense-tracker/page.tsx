...
export default function ExpenseTrackerPage() {
    const itemType = 'fruit'; // This page is for fruits
    ...
    const handleAddItem = () => {
        if(!newItemName.trim()){
            toast({ variant: 'destructive', title: 'Fruit name cannot be empty.'});
            return;
        }
        ...
        setNewItemName('');
        toast({ title: `Fruit tracker "${newItem.name}" created.` });
    }
    ...
    const handleDeleteItem = (itemIdToDelete: string) => {
        ...
        toast({ title: `Fruit tracker "${itemToDelete.name}" deleted.`});
    }

    const handleAddTransaction = (transaction: Omit<Transaction, 'id' | 'itemId'>) => {
        if (!activeItemId) {
            toast({ variant: 'destructive', title: 'Please select a fruit first.' });
            return;
        }
    ...
    <main>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div className="space-y-2">
            <h1 className="text-3xl font-bold">{t('expenseTrackerPage.title')} for Fruits</h1>
             ...
             {items.length > 0 && (
                <Select onValueChange={setActiveItemId} value={activeItemId || ''}>
                    <SelectTrigger className="w-[220px]">
                        <SelectValue placeholder="Select a fruit tracker" />
                    </SelectTrigger>
                    <SelectContent>
                        {items.map(item => (
       ...
       {items.length === 0 ? (
         <Card className="max-w-xl mx-auto my-16 text-center p-8">
            <CardHeader>
                <CardTitle>Create Your First Fruit Tracker</CardTitle>
                <CardDescription>To start tracking expenses, you need to add a fruit first. For example, "Mango Orchard 1" or "Apple Trees".</CardDescription>
            </CardHeader>
            <CardContent>
                 <div className="flex items-center gap-2 max-w-sm mx-auto">
                    <Input 
                        placeholder="e.g., Mango Orchard 1"
                        value={newItemName}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div className="lg:col-span-1 space-y-6 sticky top-20">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Manage Fruit Trackers</CardTitle>
                        </CardHeader>
                        <CardContent>
                             <div className="flex items-center gap-2">
                                <Input 
                                    placeholder="Add new fruit tracker..."
                                    value={newItemName}
                                    onChange={(e) => setNewItemName(e.target.value)}
                                />
                                <Button onClick={handleAddItem} size="icon">
                                        ...
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    ...
                                                    <AlertDialogDescription>
                                                        This will permanently delete the fruit tracker for "{item.name}" and all its associated income and expense records. This action cannot be undone.
                                                    </AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
